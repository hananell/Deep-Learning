...
# fiveGram is (contextWords, midWord)
...
# Converts context words to context words' chars
...
# Converts context words to context words' nums
...
class MyDatasetTest(Dataset):
    def __init__(self, fiveGrams):
        self.words = [fiveGramsWords(fiveGram) for fiveGram in fiveGrams]
        self.chars = [fiveGramsChars(fiveGram) for fiveGram in fiveGrams]
...
def test():
    ...
    testFiveGrams = createFiveGramsTest(testDataLowered)
    testLoader = DataLoader(MyDatasetTest(testFiveGrams))
    f = open("test5." + str(sys.argv[1]), 'w')
    with torch.no_grad():
        for context, midWordOrigin in zip(testLoader, testDataOrigin):
            # prediction
            output = modely(context)
            predLabelNum = output.max(1, keepdim=True)[1]  # get the index of the max log-probability
            # write result
            f.write(midWordOrigin + " " + labelNum2Label(predLabelNum, vocabL) + "\n")
            # new line after end of sentence
            if context[0][0][-1].item() == vocabW['PAD_END'] and context[0][0][-2].item() == vocabW['PAD_END']:
                f.write("\n")
    f.close()
